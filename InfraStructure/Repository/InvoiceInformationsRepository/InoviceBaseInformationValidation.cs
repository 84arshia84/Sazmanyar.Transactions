...
public class InoviceBaseInformationValidation
{
    private readonly AppDbContext _context;
    public InoviceBaseInformationValidation(AppDbContext context)
    {
        _context = context;
    }
    public async Task<(string message, bool isSuccess, Guid invoiceBaseInformationId)> Validation(InvoiceBaseInformation invoiceBaseInformation)
    {
        try
        {
            ...
            if (invoiceBaseInformation.InvoiceTypeId == prepaymentGuid)
            {
                ...
                return ("", true, invoiceBaseInformation.Id);


        }
        catch (Exception ex)
        {
            return ($"ValidationError: {ex.Message}", false, Guid.Empty);
        }
    }
}
